...
// Clean out previously malformed additions from src/data/recipes.ts
let content = fs.readFileSync(path.join(projectRoot, 'src', 'data', 'recipes.ts'), 'utf8');

// Find end of original 109 recipes before the added ones
const originalEndMarker = "    slug: 'guava-gin-cooler'";
const markerIndex = content.indexOf(originalEndMarker);
if (markerIndex !== -1) {
  const nextObjEnd = content.indexOf('  }', markerIndex);
  if (nextObjEnd !== -1) {
    const cleanPrefix = content.slice(0, nextObjEnd + 3);
    
    // Format and append the new recipes cleanly
    const appendedFormatted = formattedMasterRecipes.map(r => ',\n  ' + JSON.stringify(r, null, 2).replace(/"([^"]+)":/g, '$1:')).join('');
    const newContent = cleanPrefix + appendedFormatted + '\n];\n';
    fs.writeFileSync(path.join(projectRoot, 'src', 'data', 'recipes.ts'), newContent, 'utf8');
    console.log(`Successfully formatted and written ${formattedMasterRecipes.length} master recipes into src/data/recipes.ts`);
  }
}
