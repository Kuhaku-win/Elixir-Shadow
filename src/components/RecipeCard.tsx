...
export default function RecipeCard({ recipe }: RecipeCardProps) {
  ...
  const { elementRef, handleMouseMove } = useMouseSpotlight<HTMLAnchorElement>();
  ...
  return (
    <a
      ...
      className="group relative flex flex-col rounded-2xl bg-obsidian-850/90 border border-gold-500/20 overflow-hidden hover:border-gold-500/60 transition-all duration-500 hover:shadow-gold-glow hover:-translate-y-1.5 backdrop-blur-md card-spotlight"
    >
      {/* Top Image Box */}
      <div className="relative h-56 w-full overflow-hidden bg-obsidian-900 flex items-center justify-center">
          ...
          <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-obsidian-800 via-obsidian-900 to-obsidian-950 p-4 text-center">
            <div className="transition-transform duration-300 ease-out group-hover:scale-110">
              <ChibiGlassIcon
                ...
                size={68}
                className="mb-1.5 transition-transform duration-500"
              />
            </div>
            <span className="text-xs font-serif text-slate-300 font-medium">{recipe.name}</span>
            <span className="text-[10px] font-mono text-slate-500 mt-0.5">{recipe.glass.split('/')[0]}</span>
          </div>
        )}
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-1.5 z-10">
          <span className="text-[10px] font-serif tracking-wider px-2.5 py-0.5 rounded-md bg-obsidian-950/90 text-slate-200 border border-white/10 backdrop-blur-md">
            {recipe.baseSpiritZh}
          </span>
          {recipe.isIbaCertified && (
            <span className="text-[10px] font-mono tracking-wider uppercase px-2 py-0.5 rounded-md bg-obsidian-950/90 text-gold-300/90 border border-gold-500/25 backdrop-blur-md">
              IBA Official
            </span>
          )}
          {recipe.category === 'competition' && (
            <span className="text-[10px] font-mono tracking-wider uppercase px-2 py-0.5 rounded-md bg-obsidian-950/90 text-amber-300/90 border border-amber-500/25 backdrop-blur-md">
              Champion Spec
            </span>
          )}
        </div>
        ...
        <div className="absolute top-3 right-3 flex items-center gap-1.5 z-10">
          <button
            ...
            className={`p-1.5 rounded-lg border backdrop-blur-md transition-all duration-300 ${
              isFav
                ? 'bg-rose-500/30 border-rose-500/60 text-rose-400 shadow-sm scale-110'
                : 'bg-obsidian-950/80 border-white/10 text-slate-400 hover:text-rose-300 hover:border-rose-500/40 hover:scale-105'
            }`}
            title={isFav ? '取消收藏' : '加入我的收藏'}
          >
            <Heart className={`w-3.5 h-3.5 ${isFav ? 'fill-rose-400 text-rose-400' : ''}`} />
          </button>
          <span className="text-[11px] font-mono font-bold px-2 py-0.5 rounded-md bg-obsidian-950/85 text-amber-400 border border-amber-500/30 backdrop-blur-md shadow-sm">
            {recipe.abv}% ABV
          </span>
        </div>
        ...
        <div className="absolute bottom-2.5 left-3 right-3 flex items-center justify-between text-[11px] text-slate-300 z-10">
          <span className="truncate max-w-[65%] text-slate-400 font-mono text-[10px]">{recipe.glass.split('/')[0]}</span>
          <span className="px-2 py-0.5 rounded-full bg-obsidian-950/80 text-slate-300 border border-white/10 text-[10px] font-mono">
            {recipe.techniqueZh}
          </span>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 font-sans tracking-wide mb-2.5 opacity-80 group-hover:opacity-100 transition-opacity">
            {recipe.nameEn}
          </p>
        ...
        <div className="pt-3 border-t border-white/5 flex flex-wrap items-center justify-between gap-1">
          <div className="flex flex-wrap gap-1">
            {recipe.flavorProfiles.map((f, idx) => (
              <span
                key={`${recipe.slug}-${f}-${idx}`}
                className="text-[10px] px-2 py-0.5 rounded-full bg-obsidian-750/90 text-slate-400 border border-white/5 group-hover:border-gold-500/20 transition-colors"
              >
                {f}
              </span>
            ))}
          </div>
          <span className="text-[11px] font-serif text-gold-400/80 group-hover:text-gold-300 font-medium group-hover:translate-x-1 transition-transform inline-flex items-center gap-0.5">
            配方详情 &rarr;
          </span>
        </div>
      </div>
