...
| **测试套件** | **Vitest** | 8 个测试套件、49 项自动化测试（覆盖数据外键、拼音检索、平替算法、稀释物理模型等） |
...
```mermaid
graph TD
  A[Elixir & Shadow 核心数据枢纽] --> B[配方数据库 Recipes (107款)]
  A --> C[原料百科数据库 Ingredients (95种)]
  A --> D[大师与冠军作品 Masters & Competitions]
  A --> E[冷知识与历史逸事 Trivia & Lore]
...
| 维度 | 指标参数 | 覆盖范围与特色 |
| :--- | :--- | :--- |
| **精选酒谱库** | **107 款** | 涵盖 IBA 官方认证三大名录、世界大赛冠军作品、传世百年经典、现代先锋特调、家庭高频果汁汽水系列及 Mocktail 零酒精特调 |
| **原料百科库** | **95 种** | 涵盖六大基酒、利口酒、纯果汁、手工糖浆、汽水辅料、苦精、调理剂及装饰水果，均附选购、保存与风味指南 |
| **原料平替体系** | **100% 关键辅料覆盖** | 每种常见原料均配置 3 级推荐梯队（优选、备选、应急）及风味换算提示 |
| **静态路由总数** | **242 个页面** | 首页、404、配方详情 (107)、原料详情 (95)、吧台、工坊、派对、收藏、大师、学堂、主题等全量静态化 |
